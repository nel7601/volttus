...
async function login(username: string, password: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`Alectra login failed: HTTP ${res.status}`)

  const body = (await res.json()) as AlectraLoginResponse
  if (!body.data?.accessToken) throw new Error("Alectra login: no access token in response")
...
async function fetchCurrentBill(
  ...
  if (!res.ok) throw new Error(`Alectra current bill failed: HTTP ${res.status}`)

  const body = (await res.json()) as AlectraBillResponse
  if (!body.data) throw new Error("Alectra current bill: no data in response")
